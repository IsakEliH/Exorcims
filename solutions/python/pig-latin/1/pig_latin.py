def translate(text: str):
    ...
    def followed_by(word: str, follower: str, one: bool) -> bool:
        ...
        if one:
            ...
            return (
                word[next_letter - 1] == follower[0]
                and word[next_letter] == follower[1]
            )
    ...
    def run(split_text: list[str]) -> str:
        ...
        for word in split_text:
            if rule1(word):
                final_string.append(word + ADD)

            elif rule2(word):
                new_word = move_consonants(word)
                final_string.append(new_word + ADD)

            elif rule3(word):
                new_word = move_consonants(word)
                qu_word = qu_check(new_word)
                final_string.append(qu_word + ADD)
